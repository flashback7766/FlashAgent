        ...
        if effective_profile != ToolsetProfile::Compact {
            ...
            if self.web_enabled.load(Ordering::Relaxed) {
                specs.push(ToolSpec {
                    ...
                    parameters_json: r#"{"type":"object","properties":{"url":{"type":"string"}},"required":["url"]}"#.into(),
                });
                specs.push(ToolSpec {
                    name: "web_search".into(),
                    description: "Search the web for documentation, articles, and solutions".into(),
                    parameters_json: r#"{"type":"object","properties":{"query":{"type":"string"},"count":{"type":"integer","description":"Results to return (default 5)"}},"required":["query"]}"#.into(),
                });
            }
        }
