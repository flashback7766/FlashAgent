...
- [~] **A11. Any OpenAI-compatible provider** — whatever speaks `/chat/completions` works, local or cloud, not only the ones with a preset. *Presets: LM Studio, Ollama, vLLM, llama.cpp, OpenRouter, OpenAI, DeepSeek, Mistral, Groq, Gemini. The client adapts to the server instead of assuming one: a request field the server names in a 400/422 is left out from then on (`top_k`, `max_tokens` → `max_completion_tokens`, …), 429/502/503/504/529 are retried after `Retry-After`, tool calls without `index` (Gemini) stay apart. Open for v1.0: each preset checked end to end with a real key — streaming, tool calls, reasoning settings, context from the model list. The native Anthropic protocol moves to v1.x.*
